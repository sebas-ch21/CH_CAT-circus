...
export function ManagerCenter() {
  ...
  const updateDateRange = (preset) => {
    ...
    if (preset === 'this_week') {
      const day = now.getDay() || 7; start.setDate(now.getDate() - day + 1); end.setDate(start.getDate() + 6);
    } else if (preset === 'last_week') {
      const day = now.getDay() || 7; start.setDate(now.getDate() - day - 6); end.setDate(start.getDate() + 6);
  ...
  const handleScheduleChange = (timeVal, attendeesStr) => {
    setTeamSchedule(prev => ({ ...prev, [timeVal]: attendeesStr === '' ? '' : parseInt(attendeesStr) }));
  };
        ...
        {activeTab === 'dispatch' && (
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 flex-1">
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                {queue.map(entry => (
                  <button key={entry.id} onClick={() => setSelectedIC(entry)} className={`w-full text-left p-4 rounded-xl border-2 transition-all ${selectedIC?.id === entry.id ? 'border-[#5E4791] bg-purple-50 shadow-md ring-4 ring-purple-500/20' : 'border-gray-100 hover:border-gray-300 bg-gray-50'}`}>
                    ...
                    <div className="text-xs font-bold text-red-500 flex items-center gap-1.5 bg-red-50 w-fit px-2 py-1 rounded-md border border-red-100">
            ...
            <div className="xl:col-span-4 bg-[#0F172A] rounded-2xl p-6 shadow-xl text-white flex flex-col h-[700px] relative overflow-hidden">
              ...
              <button onClick={openConfirmModal} disabled={!selectedIC || !selectedSlot} className="w-full mt-auto py-5 rounded-xl font-black text-lg transition-all disabled:opacity-50 disabled:bg-gray-700 disabled:text-gray-400 bg-white text-[#0F172A] hover:bg-gray-200 shadow-lg">
                Initiate Dispatch
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 max-w-3xl mx-auto w-full">
            <div className="sticky top-0 z-20 bg-white pt-2 pb-4 border-b border-gray-200 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 shadow-sm">
              <div>
                ...
                <p className="text-gray-500 font-medium mt-1">Log the total BPS appointments your team handles per interval.</p>
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
              <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-4 border-b border-gray-100 pb-6">
                <div>
                  <h2 className="text-2xl font-black text-[#0F172A] mb-2 flex items-center gap-2"><BarChart3 className="w-6 h-6 text-[#007C8C]" /> Dispatch Statistics</h2>
                  <p className="text-gray-500 font-medium">Performance metrics based on permanently logged successful dispatches.</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(stats.byIC).sort((a,b)=>b[1]-a[1]).map(([email, count]) => (
                  <div key={email} className="flex justify-between items-center p-4 border-2 border-gray-100 rounded-xl bg-gray-50 hover:border-gray-300 transition-colors">
