...
export function ManagerCenter() {
  ...
  const updateDateRange = (preset) => {
    ...
    setStatsStart(start.toISOString().split('T')[0]); setStatsEnd(end.toISOString().split('T')[0]);
  };

  const loadStatistics = async () => {
    if (!statsStart || !statsEnd) return;
    const startIso = new Date(`${statsStart}T00:00:00`).toISOString();
    const endIso = new Date(`${statsEnd}T23:59:59`).toISOString();
    const { data: logs } = await supabase.from('dispatch_logs').select('*').gte('matched_at', startIso).lte('matched_at', endIso);
    if (logs) {
      const byIC = {}, byTier = { 1: 0, 2: 0, 3: 0 };
      logs.forEach(log => { byIC[log.ic_email] = (byIC[log.ic_email] || 0) + 1; byTier[log.tier_rank] = (byTier[log.tier_rank] || 0) + 1; });
  ...
  const handleScheduleChange = (timeVal, attendeesStr) => { setTeamSchedule(prev => ({ ...prev, [timeVal]: attendeesStr === '' ? '' : parseInt(attendeesStr) })); };

  const saveMySchedule = async () => {
    setSavingSchedule(true);
    await supabase.from('manager_schedules').upsert({ manager_email: user.email, schedule_date: scheduleDate, schedule_data: teamSchedule, updated_at: new Date().toISOString() }, { onConflict: 'manager_email, schedule_date' });
    setSavingSchedule(false);
    toast.success('Schedule saved');
  };

  const openConfirmModal = () => { setZoomLinkInput(selectedSlot?.zoom_link || ''); setShowConfirmModal(true); };
  ...
  const executeDispatch = async () => {
    ...
    setDispatching(true);
    try {
      const icId = selectedIC.ic_id;

      // 1. Explicit Error Checking on Update
      ...
      if (slotErr) throw new Error(`Failed to assign slot: ${slotErr.message}`);

      // 2. Mark IC Profile as BUSY
      await supabase.from('profiles').update({ current_status: 'BUSY' }).eq('id', icId);
      
      // 3. Attempt queue cleanup locally
      await supabase.from('queue_entries').delete().eq('ic_id', icId);

      toast.success('IC successfully dispatched!');
      setSelectedIC(null); setSelectedSlot(null); setShowConfirmModal(false); 
  ...
  const handleSendBackToQueue = async (slot) => {
    ...
    try {
      ...
      // Drop Slot back to OPEN
      const { error: resetErr } = await supabase.from('bps_slots').update({ status: 'OPEN', assigned_ic_id: null, assigned_at: null }).eq('id', slot.id);
      ...
      toast.success('Slot Reset. IC returned to queue.');
      fetchData();
    } catch (error) { 
  ...
  const getTabClass = (id) => `px-6 py-4 font-bold text-sm transition-all border-b-4 focus:outline-none ${activeTab === id ? 'border-[#5E4791] text-[#0F172A] bg-white' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`;
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-20 font-sans relative">
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-8 w-full flex-1 flex flex-col">
        <div className="flex justify-between items-end mb-6">
          <div><h1 className="text-3xl font-black text-[#0F172A]">Manager Center</h1><p className="text-gray-500 mt-1 font-medium">Live dispatching, team schedules, and performance tracking.</p></div>
        ...
        <div className="flex rounded-t-2xl border border-gray-200 overflow-hidden shadow-sm mb-6 bg-white">
          <button onClick={() => setActiveTab('dispatch')} className={getTabClass('dispatch')}><div className="flex items-center gap-2"><Clock className="w-4 h-4"/> Live Dispatch</div></button>
        ...
        {activeTab === 'dispatch' && (
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 flex-1">
            <div className="xl:col-span-4 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col h-[700px]">
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100"><h2 className="font-bold text-[#0F172A] flex items-center gap-2"><User className="w-5 h-5 text-[#5E4791]" /> Waiting Queue</h2><span className="bg-[#F3EFF9] text-[#5E4791] px-3 py-1 rounded-full text-xs font-black">{queue.length}</span></div>
              <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                {queue.map(entry => (
                  <button key={entry.id} onClick={() => setSelectedIC(entry)} className={`w-full text-left p-4 rounded-xl border-2 transition-all ${selectedIC?.id === entry.id ? 'border-[#5E4791] bg-purple-50 shadow-md ring-4 ring-purple-500/20' : 'border-gray-100 hover:border-gray-300 bg-gray-50'}`}>
                    <div className="flex justify-between items-center mb-3"><span className="font-black text-lg text-[#0F172A] truncate">{entry.profiles?.email?.split('@')[0]}</span><span className="bg-gray-800 text-white text-[10px] uppercase tracking-widest font-bold px-2 py-1 rounded-md shadow-sm">Tier {entry.profiles?.tier_rank}</span></div>
                    <div className="text-xs font-bold text-red-500 flex items-center gap-1.5 bg-red-50 w-fit px-2 py-1 rounded-md border border-red-100"><Clock className="w-3 h-3" /> Wait: {Math.round((new Date() - new Date(entry.entered_at)) / 60000)} mins</div>
                  </button>
                ))}
                {queue.length === 0 && <div className="text-center py-10 text-gray-400 font-medium border-2 border-dashed border-gray-200 rounded-xl">Queue is empty.</div>}
              </div>
            </div>

            <div className="xl:col-span-4 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col h-[700px]">
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100"><h2 className="font-bold text-[#0F172A] flex items-center gap-2"><Calendar className="w-5 h-5 text-[#007C8C]" /> Open Slots</h2><span className="bg-[#E0F5F6] text-[#007C8C] px-3 py-1 rounded-full text-xs font-black">{openSlots.length}</span></div>
              <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                {openSlots.map(slot => {
                  ...
                  return (
                    <div key={slot.id} className={`w-full text-left p-4 rounded-xl border-2 transition-all flex flex-col ${selectedSlot?.id === slot.id ? 'border-[#007C8C] bg-[#E0F5F6] shadow-md ring-4 ring-[#007C8C]/20' : 'border-gray-200 hover:border-gray-300 bg-white'}`}>
                      <div onClick={() => setSelectedSlot(slot)} className="cursor-pointer">
                        <div className="flex justify-between items-start mb-3"><div className="font-black text-lg text-[#0F172A]">{slot.patient_identifier}</div>{slot.host_manager && <span className="text-[#007C8C] font-bold text-[9px] uppercase tracking-widest bg-white border border-[#007C8C]/20 shadow-sm px-2 py-1 rounded-md">Host: {slot.host_manager.split('@')[0]}</span>}</div>
                        <div className="flex justify-between items-center bg-gray-50 rounded-lg p-2 mb-3 border border-gray-100">
                          <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center flex-1 border-r border-gray-200"><span className="block text-gray-400 mb-0.5">BPS</span>{times.bps}</div>
                          <div className="text-[10px] font-bold text-[#5E4791] uppercase tracking-widest text-center flex-1"><span className="block text-purple-300 mb-0.5">Overflow</span>{times.of}</div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="xl:col-span-4 bg-[#0F172A] rounded-2xl p-6 shadow-xl text-white flex flex-col h-[700px] relative overflow-hidden">
              ...
              <div className="space-y-4 flex-1">
                <div className="bg-white/5 p-5 rounded-xl border border-white/10"><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><User className="w-3 h-3"/> Selected Staff (IC)</p>{selectedIC ? <p className="font-bold text-xl text-white">{selectedIC.profiles.email.split('@')[0]}</p> : <p className="text-gray-500 italic font-medium">Waiting for selection...</p>}</div>
                <div className="bg-white/5 p-5 rounded-xl border border-white/10"><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><Calendar className="w-3 h-3"/> Selected Room (Slot)</p>{selectedSlot ? (<div><p className="font-bold text-xl text-white mb-2">{selectedSlot.patient_identifier}</p><div className="bg-black/30 rounded-lg p-3 text-sm font-medium">OF Time: <span className="text-purple-300 font-bold">{getDualTimes(selectedSlot.start_time).of}</span></div></div>) : <p className="text-gray-500 italic font-medium">Waiting for selection...</p>}</div>
              </div>
              <button onClick={openConfirmModal} disabled={!selectedIC || !selectedSlot} className="w-full mt-auto py-5 rounded-xl font-black text-lg transition-all disabled:opacity-50 disabled:bg-gray-700 disabled:text-gray-400 bg-white text-[#0F172A] hover:bg-gray-200 shadow-lg">Initiate Dispatch</button>
            </div>
          </div>
        )}

        {/* RE-QUEUE BUTTON LOGIC FOR MANAGERS */}
        {activeTab === 'scheduled' && (
                          ...
                          <td className="p-4">
                            {slot.status === 'ASSIGNED' ? (
                              <div className="flex items-center gap-3">
                                <span className="text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-md border bg-yellow-50 text-yellow-700 border-yellow-200">PENDING (5m)</span>
                                <button onClick={() => handleSendBackToQueue(slot)} disabled={dispatching} className="px-3 py-1.5 bg-white hover:bg-red-50 text-red-600 text-xs font-bold rounded-lg border-2 border-gray-200 hover:border-red-200 transition-colors shadow-sm disabled:opacity-50">
                                  Cancel & Re-Queue
                                </button>
                              </div>
                            ) : (
        ...
        {activeTab === 'team' && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 max-w-3xl mx-auto w-full">
            <div className="sticky top-0 z-20 bg-white pt-2 pb-4 border-b border-gray-200 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 shadow-sm">
              <div><h2 className="text-2xl font-black text-[#0F172A] flex items-center gap-2"><Users className="w-6 h-6 text-[#5E4791]" /> My Team Schedule</h2></div>
              <div className="flex items-end gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                <div><label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Target Date</label><input type="date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)} className="px-3 py-2 border-2 border-gray-200 rounded-lg font-black text-[#0F172A] focus:border-[#5E4791] outline-none" /></div>
                <button onClick={saveMySchedule} disabled={savingSchedule} className="bg-[#0F172A] text-white font-black px-4 py-2.5 rounded-lg shadow-md hover:bg-gray-800 disabled:opacity-50 transition-all flex items-center gap-2 h-[42px]">{savingSchedule ? <Loader className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4"/> Save</>}</button>
              </div>
            </div>
            <div className="space-y-1">
              <div className="flex font-black text-[10px] text-gray-400 uppercase tracking-widest px-4 pb-2 border-b border-gray-200 mb-2"><div className="flex-1">Time Interval (MT)</div><div className="w-32 text-center">Team BPS Count</div></div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
              <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-4 border-b border-gray-100 pb-6">
                <div><h2 className="text-2xl font-black text-[#0F172A] mb-2 flex items-center gap-2"><BarChart3 className="w-6 h-6 text-[#007C8C]" /> Dispatch Statistics</h2></div>
                <div className="flex flex-wrap items-center gap-4 bg-gray-50 p-3 rounded-xl border border-gray-200">
                  <select value={statsPreset} onChange={(e) => updateDateRange(e.target.value)} className="px-4 py-2 border-2 border-gray-200 rounded-lg font-bold text-[#0F172A] focus:border-[#007C8C] outline-none">
                    <option value="last_7_days">Last 7 Days</option><option value="this_week">This Week</option><option value="last_week">Last Week</option><option value="this_month">This Month</option><option value="last_month">Last Month</option><option value="custom">Custom Range...</option>
                  </select>
                  <div className="flex items-center gap-2"><input type="date" value={statsStart} onChange={(e) => { setStatsPreset('custom'); setStatsStart(e.target.value); }} className="px-3 py-2 border-2 border-gray-200 rounded-lg font-bold text-gray-600 outline-none focus:border-[#007C8C]" /><span className="text-gray-400 font-bold">to</span><input type="date" value={statsEnd} onChange={(e) => { setStatsPreset('custom'); setStatsEnd(e.target.value); }} className="px-3 py-2 border-2 border-gray-200 rounded-lg font-bold text-gray-600 outline-none focus:border-[#007C8C]" /></div>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div className="bg-[#F3EFF9] border border-[#E7DFF3] rounded-2xl p-6 text-center shadow-sm"><p className="text-xs font-black text-[#5E4791] uppercase tracking-widest mb-2">Total Dispatches</p><p className="text-5xl font-black text-[#0F172A]">{stats.total}</p></div>
                <div className="bg-[#E0F5F6] border border-[#C1ECEF] rounded-2xl p-6 text-center shadow-sm"><p className="text-xs font-black text-[#007C8C] uppercase tracking-widest mb-2">Tier 1 Dispatches</p><p className="text-5xl font-black text-[#0F172A]">{stats.byTier[1] || 0}</p></div>
                <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 text-center shadow-sm"><p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Tier 2/3 Dispatches</p><p className="text-5xl font-black text-[#0F172A]">{(stats.byTier[2] || 0) + (stats.byTier[3] || 0)}</p></div>
              </div>
              <h3 className="text-lg font-black text-[#0F172A] mb-4">Total Dispatches per IC</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(stats.byIC).sort((a,b)=>b[1]-a[1]).map(([email, count]) => (<div key={email} className="flex justify-between items-center p-4 border-2 border-gray-100 rounded-xl bg-gray-50"><span className="font-bold text-gray-700 truncate pr-4">{email.split('@')[0]}</span><span className="bg-[#0F172A] text-white px-3 py-1 rounded-lg font-black">{count}</span></div>))}
              </div>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-8">
              <div className="bg-gray-50 border-2 border-gray-100 rounded-2xl p-5 shadow-sm space-y-4 mb-6">
                <div className="flex justify-between items-center pb-4 border-b border-gray-200"><span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Staff Member</span><span className="font-black text-[#007C8C] text-lg">{selectedIC.profiles.email.split('@')[0]}</span></div>
                <div className="flex justify-between items-center pb-4 border-b border-gray-200"><span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Room ID</span><span className="font-black text-[#0F172A] text-lg">{selectedSlot.patient_identifier}</span></div>
                <div className="flex justify-between items-center"><span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Overflow Time</span><span className="font-black text-[#5E4791] text-lg bg-purple-50 px-3 py-1 rounded-lg">{getDualTimes(selectedSlot.start_time).of}</span></div>
              </div>
              <div>
